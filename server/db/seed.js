...
const seedDatabase = async () => {
  try {
    ...
    console.log('Connected to MongoDB for dataset seeding...');

    // Clear existing problems and topics
    ...
    for (const [topicName, items] of Object.entries(rawData)) {
      ...
      const problemsToInsert = items.map((item, idx) => ({
        topic_id: topic._id,
        title: item[0],
        difficulty: item[1],
      ...
      totalProblemsCount += problemsToInsert.length;
    }

    console.log(`Successfully seeded ${Object.keys(rawData).length} topics and ${totalProblemsCount} problems into CodeSolver!`);
    return { topics: Object.keys(rawData).length, problems: totalProblemsCount };
  } catch (error) {
